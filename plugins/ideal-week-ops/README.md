...
1. **Run `setup`.** Detection-first onboarding. Walks you through Composio install (or detects your existing MCPs), picks calendar + notification tools, captures channel + recipient, surfaces the self-notification trap warning if you pick Slack to your own handle, writes `client-profile/ideal-week-ops.local.md` in your workspace. Resumable mid-flow.
...
- **`setup`** — *neutral.* One-time onboarding wizard. Detects whether the host's AI has calendar-read and notification-send capabilities wired (via Composio MCP, direct vendor MCPs, or any other source). Walks the user through wiring if missing. Captures notification channel + recipient. Surfaces the self-notification trap on Slack (warning, not blocker). Writes `client-profile/ideal-week-ops.local.md`. Resumable. Re-runnable to refresh any field.
...
- **Schedule cadence.** Default twice daily (weekdays 5pm + 7am). Adjust in `client-profile/ideal-week-ops.local.md` (`scan_schedule:`). Wire the actual recurrence in your runtime's scheduling mechanism — see Suggested tool wiring.
...
**`scan-ideal-week` reports "no ideal week documented".** The scan skill reads from the path configured in `client-profile/ideal-week-ops.local.md` (default `client-profile/ideal-week.md`). Run `extract-ideal-week` first, or update the config path if the document lives elsewhere.

---

**You see a "Migrated your wiring config" message on setup, or a "config is at the old `.claude/` path" error on `extract-ideal-week` / `scan-ideal-week`.**
Earlier versions of ideal-week-ops wrote the wiring config to `.claude/ideal-week-ops.local.md`. The current version writes to `client-profile/ideal-week-ops.local.md`. On the next `setup` run, the skill auto-migrates the file forward — copying contents to the new path and leaving the old file in place (non-destructive). If you see the error on a work skill, just run `/ideal-week-ops:setup` once. You can delete `<workspace>/.claude/ideal-week-ops.local.md` by hand once you've confirmed everything works. The legacy scan log (`.claude/ideal-week-ops.scan-log.jsonl`) is NOT migrated — the scan log starts fresh at the new path. Delete the legacy scan log when convenient.
